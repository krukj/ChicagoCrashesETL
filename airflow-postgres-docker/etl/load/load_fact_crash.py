...
def load_fact_crash(filepath_in) -> None:
    try:
        ...
        # dla pewności bo się wywala
        dups = fact_crash.duplicated(subset=["FACT_CRASH_KEY"]).sum()
        print(f"Jest {dups} powtarzających się FACT_CRASH_KEY.")
        print("!!!")
        ...
        logger.info(f"{module_tag} Bulk inserting into staging.fact_crash.")
        records = fact_crash[[
            "FACT_CRASH_KEY", "CRASH_RECORD_ID", "NUM_UNITS", "INJURIES_TOTAL",
            "INJURIES_FATAL", "INJURIES_INCAPACITATING", "INJURIES_NON_INCAPACITATING",
            "INJURIES_REPORTED_NOT_EVIDENT", "INJURIES_NO_INDICATION", "INJURIES_UNKNOWN",
            "date_id", "CRASH_INFO_KEY", "LOCATION_KEY", "PERSON_ID", "VEHICLE_ID"
        ...
        cursor.close()
        conn.close()
        logger.info(f"{module_tag} Successfully loaded fact_crash into database.")
