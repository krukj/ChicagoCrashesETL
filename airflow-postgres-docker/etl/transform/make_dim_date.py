...
def make_dim_date(
    ...
    Args:
        start_date (datetime.date): The start date for generating the dimension table. Defaults to datetime.date(2016, 1, 1).
        end_date (datetime.date): The end date for generating the dimension table. Defaults to datetime.date(2021, 12, 31).

    Returns:
        pd.DataFrame: A DataFrame containing date-related information for each date within the specified range.
    """

    us_holidays = holidays.UnitedStates(years=range(start_date.year, end_date.year))
